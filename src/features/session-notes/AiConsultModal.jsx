...
export function AiConsultModal({ consult, onClose, onSave }) {
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="ai-consult-title">
        ...
        <div className="warning-box">
          ...
          <p>
            La IA puede ayudar a redactar, pero no sustituye el criterio del fisioterapeuta.
            Revisa el contenido antes de guardarlo en el expediente.
          </p>
        </div>
        ...
        <label>
          ...
          <textarea rows="3" value={validationNotes} onChange={(e) => setValidationNotes(e.target.value)} placeholder="Ej. contenido revisado y ajustado al caso clinico..." />
        </label>
        ...
        <label className="checkbox-label">
          <input type="checkbox" checked={alsoInsert} onChange={(e) => setAlsoInsert(e.target.checked)} />
        ...
        <div className="actions">
          ...
          <button type="button" onClick={save} disabled={saving}>{saving ? 'Guardando...' : 'Guardar IA trazable'}</button>
        </div>
      </section>
    </div>
  );
